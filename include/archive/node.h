...
/** @brief process-local representation identity를 allocation 없이 발급한다. */
inline std::uint64_t NextNodeIdentity() noexcept {
...
constexpr ValueType operator|(ValueType lhs, ValueType rhs) {
  return static_cast<ValueType>(static_cast<std::uint32_t>(lhs) |
                                static_cast<std::uint32_t>(rhs));
}
/**
 * @brief ValueType 플래그를 비트 OR로 누적한다.
 * @param lhs 누적 대상(결과가 대입된다).
 * @param rhs 추가할 플래그.
 * @return 갱신된 a에 대한 참조.
 */
constexpr ValueType& operator|=(ValueType& lhs, ValueType rhs) {
 ...
 */
constexpr bool HasFlag(ValueType value, ValueType flag) {
 ...
 *  - Compose가 해소한 별칭은 값 복사본이 같은 Identity를 공유한다. 따라서
 *    값 저장은 트리 형태를 유지하면서도 representation graph의 공유 노드
 *    identity를 Serialize 단계까지 보존한다.
...
struct Node {
  ...
  /** @brief 값 복사본 사이에서도 공유되는 representation identity token. */
  std::uint64_t identity = NextNodeIdentity();
  /** @brief 노드의 종류. */
  ...
  /** @brief kScalar: 디코드된 문자열 값(UTF-8). */
  std::string scalar;
  /** @brief kBinary: DLL이 소유하는 디코드된 바이트열. */
  ...
  /** @brief 두 값이 representation graph의 같은 노드를 가리키는지 검사한다. */
  [[nodiscard]] bool SameIdentity(const Node& other) const noexcept {

 ...
 * 조회할 수 있게 합니다. 아키텍처 판별은 CMake가 생성하는
 * bedrock_common_config.h 의 매크로에 전적으로 의존합니다.
 */
#pragma once
#include <array>
#include <cstdint>
...
/**
 * @brief 아키텍처 인식(Architecture detection).
 *
 * 아키텍처 인식은 전적으로 CMake가 담당합니다(.cmake/architecture.cmake).
 * CMake가 bedrock_common_config.h 를 생성하여 BEDROCK_ARCH_X86 /
 * BEDROCK_ARCH_ARM / BEDROCK_ARCH_NAME 매크로를 주입하고, 이 헤더는
 * 그 값을 그대로 신뢰합니다. (소스 차원의 컴파일러 사전정의 매크로
 * 감지는 사용하지 않습니다.) bedrock_common_config.h 는 빌드
 * 디렉터리에 생성되어 SYSTEM PUBLIC 인클루드 경로로 노출되므로, 이
 * 라이브러리를 사용하는 측에도 전이적으로 제공됩니다.
 */
 ...
 *   - 그 외   : 미구현 → 0
 *
 * @note <windows.h>는 매크로 오염(min/max 등)이 심해 헤더에서
 *       include하지 않고 intrinsics.cc 내부에서만 include합니다.
 */
#if BEDROCK_ARCH_ARM
